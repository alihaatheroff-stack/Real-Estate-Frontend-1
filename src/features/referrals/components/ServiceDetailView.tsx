...
function downloadServiceSummary(service: Service, providerName?: string) {
  const lines = [
    service.title,
    '',
    ...
    `Rating: ${formatRating(service.rating)} (${service.reviewCount} reviews)`,
    providerName ? `Seller: ${providerName}` : null,
    '',
    'Description',
    service.description,
  ...
  const slug = service.title
    ...
    .replace(/^-|-$/g, '')
  anchor.href = url
  anchor.download = `${slug || service.id}-service.txt`
  anchor.click()
...
export function ServiceDetailView({ service }: { service: Service }) {
  ...
  const faqs = service.faqs ?? getDefaultServiceFaqs()
  const reviews = listReviews()
  const reviewLimit = Math.max(1, Math.min(service.reviewCount, reviews.length))
  const displayReviews = listReviewsForDisplay(reviewLimit)
  ...
  return (
    <div className="pb-10">
      <div className="relative">
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute inset-x-0 top-0 h-[200px] overflow-hidden bg-freeio-detail sm:h-[220px] lg:h-[280px]"
        >
        ...
        >
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 lg:mb-4">
            <nav className="flex flex-wrap items-center gap-1 text-xs text-muted sm:text-sm">
              <Link to={PATHS.home} className="hover:text-brand">
                Home
              </Link>
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              <button
                type="button"
                onClick={() => downloadServiceSummary(service, provider?.name)}
                className="inline-flex h-8 items-center gap-1.5 rounded-full border border-line bg-paper px-3 text-xs font-medium text-ink transition hover:border-brand sm:h-9 sm:px-3.5 sm:text-sm"
              >
            ...
            <div className="min-w-0">
              <div className="grid grid-cols-1 gap-5 border-y border-line/80 py-5 sm:grid-cols-3 sm:items-center sm:gap-6 lg:py-6">
                <div className="flex items-center gap-3.5">
                  <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-freeio-detail-icon">
